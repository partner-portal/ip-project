 ...
 * @copyright (c) 2017-2023, Prove & Run and/or its affiliates.
 *   All rights reserved.
 */
...
#define TZ_IOCTL_SEND_PROHIBITED      3  /* To be replaced by TZ_IOCTL_SEND_REQ+TZ_IOCTL_WAIT_RESP */
#define TZ_IOCTL_CONFIG_PROHIBITED    4  /* TZ_IOCTL_CONFIG_BY_NAME is the only available config method */
#define TZ_IOCTL_STATUS               5
#define TZ_IOCTL_CONFIG_BY_NAME       6
#define TZ_IOCTL_SEND_EXT_PROHIBITED  7 /* To be replaced by TZ_IOCTL_SEND_REQ+TZ_IOCTL_WAIT_RESP */
...
static bool _device_registered;
...
static const struct vm_operations_struct pnc_mmap_vm_ops = {
		.open =     pnc_vma_open,
		.close =    pnc_vma_close,
};
...
{
	struct pnc_session *s = filp->private_data;
...
{
	return pnc_session_open_with_flags((struct pnc_session **)&filp->private_data,
		filp->f_flags);
}
				     ...
				     unsigned long arg)
{
	...
	int ret = 0;
	uint32_t val;
	struct pnc_ioctl_params ioctl_params;
	...
	case TZ_IOCTL_ALLOC:
		...
		break;
	case TZ_IOCTL_CONFIG_BY_NAME:
		ret = pnc_session_config_by_name(s, NULL);
		break;
	case TZ_IOCTL_SEND_PROHIBITED:
		pr_err("This send request method is no more supported.\n");
		pr_err("Use TZ_IOCTL_SEND_REQ+TZ_IOCTL_WAIT_RESP instead.\n");
		ret = -ENOTSUPP;
		break;
	case TZ_IOCTL_SEND_EXT_PROHIBITED:
		pr_err("This send request method is no more supported.\n");
		pr_err("Use TZ_IOCTL_SEND_REQ+TZ_IOCTL_WAIT_RESP instead.\n");
		ret = -ENOTSUPP;
		break;
	case TZ_IOCTL_CONFIG_PROHIBITED:
		...
		pr_err("Use TZ_IOCTL_CONFIG_BY_NAME instead for session configuration.\n");
		ret = -ENOTSUPP;
		break;
...
static unsigned int pnc_create_sgi(irq_hw_number_t hwirq)
{
	...
	if (ic == NULL) {
		pr_warn("pnc: (%s) failed to locate %s and %s\n",
			__func__,
			CONFIG_PROVENCORE_INTERRUPT_CONTROLLER_COMPATIBLE_NODE,
			CONFIG_PROVENCORE_INTERRUPT_CONTROLLER_NODE_NAME);
		pr_warn("pnc: (%s) other options can be found in the file irqchip/irq-gic.c or the platform's dtb file",
			__func__);
		return 0;
	}
