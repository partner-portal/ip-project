...
int pnc_shm_init(void *vbase, uint64_t pbase, unsigned int nr_pages)
{
	...
	b = kmalloc(sizeof(struct pnc_shm_block), GFP_KERNEL);
	if (b == NULL)
		return -ENOMEM;
...
{
	struct pnc_shm_block *b = _blocks, *n;
...
int pnc_shm_alloc(unsigned int nr_pages, struct pnc_shm_block **block)
{
	struct pnc_shm_block *b = _blocks, *n;
	int ret = 0;
...
split:
	n = kmalloc(sizeof(struct pnc_shm_block), GFP_KERNEL);
	if (n == NULL) {
		ret = -ENOMEM;
		goto err;
	}
...
int pnc_shm_free(struct pnc_shm_block *b)
{
	if (b == NULL)
		return 0;
...
{
	struct pnc_header *header = (struct pnc_header *)_shm_base;

	if (header)
		/* Atomically check the header was initialized by S world */
		return virt_load_acquire(&header->magic) == REE_MAGIC_2;
	else
		return 0;
}
...
void pnc_shm_init_header(void)
{
	...
	/* Atomically mark the header as initialized by NS world  */
	virt_store_release(&header->magic, REE_MAGIC_1);
}
