...
//! Binary transport using `postcard` for serialization
...
pub struct PostcardSender<S: BytesSender> {
    inner: S,
}

impl<T, S> Sender<T> for PostcardSender<S>
where
    T: serde::Serialize,
    S: BytesSender,
{
    fn builder() -> fn(me: s_pid_t, other: s_pid_t) -> Self {
        |me, other| PostcardSender {
            inner: S::builder()(me, other),
        }
    }
    fn send(&self, val: &T) -> Result<()> {
        let buf = postcard::to_allocvec(val)
            .map_err(|e| Error::msg(&format!("Postcard Serialization failed: {}", e)))?;
        self.inner.send(&buf)
    }
}

pub struct PostcardReceiver<R: BytesReceiver> {
    inner: R,
}

impl<T, R> Receiver<T> for PostcardReceiver<R>
where
    T: for<'de> serde::Deserialize<'de>,
    R: BytesReceiver,
{
    fn builder() -> fn(me: s_pid_t, other: s_pid_t) -> Self {
        |me, other| PostcardReceiver {
    ...
    fn recv(&self) -> Result<T> {
        ...
        postcard::from_bytes(&buf)
            .map_err(|e| Error::msg(&format!("Postcard Deserialization failed: {}", e)))
    }
}
