...
#include "pnr_ring/light_ring.h"
#else
/* TODO REMOVE THIS INCLUDE AS SOON AS SUPPORT for pnr_ring.h will be removed */
 ...
 *
 * - 3.06:
 *     fix invalid handling of syscall interruption (ERESTARTSYS returned by
 *     wait_event_interruptible or wait_event_interruptible_timeout).
 *
 * - 3.07:
 *     clean code from old REEV2 legacy features.
 *     This mainly concerns:
 *      - removal of session configuration with SID
 *      - removal of obsolete send request commands.
 *
 * - 3.08:
 *     Update REE core with new ring buffer library: light_ring unidirectional
 *     ring buffer library.
 *     Previous one (pnr_ring.h) shall be kept supported in ProvenCore to handle
 *     backward compatibility. It is totally removed from Linux kernel REE driver.
 */
#define REE_VERSION         UINT32_C(0x308) /* 3.08 */
...
/**
 * The capacity requested for each ring buffer in num of slot, e.g struct pnc_message:
 *  - should be a power of 2 for a correct alignment of ring buffers.
 *  - should allow storage of the maximum num of struct pnc_message possible
 */
#define PNC_MESSAGE_RING_SLOT_CAPACITY  UINT32_C(0x100)

/* Assertions around PNC_MESSAGE_RING_SLOT_CAPACITY
 *
 * TODO: REMOVE 1st ASSERTION WHEN REMOVING pnr_ring.h SUPPORT since ring
 * capacities need not be powers of 2 anymore with light_ring API.
 */
...
_Static_assert(PNC_MESSAGE_RING_SLOT_CAPACITY > 0 &&
	PNC_MESSAGE_RING_SLOT_CAPACITY < (UINT32_C(1) << 31),
	"PNC_MESSAGE_RING_SLOT_CAPACITY is out of range");
...
/** TODO: REMOVE CALL TO THIS MACRO WHEN REMOVING pnr_ring.h SUPPORT
 *
...
/** TODO: REMOVE THIS STRUCTURE WHEN REMOVING pnr_ring.h SUPPORT
 *
 * Former \ref struct pnc_message_ring kept here for some temporary backward
 * compatibility features.
 */
struct pnc_message_ring_OBSOLETE {
	/* Shared part of a struct pnc_message ring buffer */
	 ...
	 */
	struct pnc_message padding[PNC_MESSAGE_RING_SLOT_CAPACITY];
};

/** TODO: REMOVE THIS STRUCTURE WHEN REMOVING pnr_ring.h SUPPORT
 *
 * Former \ref struct pnc_shm kept here for some temporary backward
 * compatibility features.
 */
struct pnc_shm_OBSOLETE {
	/** Common SHM header */
	struct pnc_header hdr;
	...
	/** NS->S ring buffer */
	struct pnc_message_ring_OBSOLETE ring_ns_to_s;

	/** S->NS ring buffer */
	struct pnc_message_ring_OBSOLETE ring_s_to_ns;
};
...
/**
 * @brief Static definition of ring buffer used to manipulate struct pnc_message
 *
 * This structure is used to statically define amount of bytes used by a ring
 * buffer in struct pnc_shm defined below.
 */
struct pnc_message_ring {
	/* Message information shared by the producer */
	lr_shrd_prod_t prod_shared;
	/* Message information shared by the consumer */
	lr_shrd_cons_t cons_shared;
	/* Ring buffer's messages slots */
	struct pnc_message msgs[PNC_MESSAGE_RING_SLOT_CAPACITY];
};
...
struct pnc_shm {
	...
	/** NS->S ring buffer */
	struct pnc_message_ring ring_ns_to_s;

	/** S->NS ring buffer */
	struct pnc_message_ring ring_s_to_ns;
};

_Static_assert((sizeof(struct pnc_shm) <= (REE_RESERVED_PAGES*PAGE_SIZE)), "not enough SHM reserved pages");
