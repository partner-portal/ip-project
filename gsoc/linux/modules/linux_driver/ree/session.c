 ...
 * @copyright (c) 2019-2023, Prove & Run S.A.S and/or its affiliates.
 *   All rights reserved.
 */
...
/* This is SID legacy used to indicate session's configuration was made "by name"
 * Today, even if configuration is always made by name, it can be that secure
 * ree service is with old code and will still expect this ID to consider config
 * "by name". So that in order to ensure backward/forward compatibility between
 * ree versions, we may continue to send it when synced REE version is not at
 * least 3.07...
 */
...
struct pnc_session {
	...
	struct pnc_shm_block *mem;

	/** Session states. */
	enum session_state global_state;
	enum session_state server_state;
	enum session_state client_state;

	/** Content of last pending message(s) */
	struct pnc_message server_message;
	struct pnc_message client_message;
...
static struct pnc_session _sessions[REE_MAX_SESSIONS];

/** Mutex to protect @_sessions accesses */
static DEFINE_MUTEX(_sessions_mutex);

/** Linux private part of the NS --> S ring buffer. */
static lr_priv_prod_t _ns_to_s_ring;

/** Linux private part of the S --> NS ring buffer. */
static lr_priv_cons_t _s_to_ns_ring;

/** Notification registers addr */
static uint32_t *_ns_to_s_notification_register;
static uint32_t *_s_to_ns_notification_register;

/** Sessions signal registers */
static pnc_signal_t *_ns_to_s_signals;
static pnc_signal_t *_s_to_ns_signals;

/** Spinlock to control access to NS-->S ring buffer. */
static DEFINE_SPINLOCK(ring_lock);

/** Session to start signal notification */
static struct pnc_session *_signal_session;
 ...
 */
static uint32_t _ree_version;

/**
 * @brief Atomically set some bits in an unsigned 32-bit integer with release
 * memory ordering and return the previous value of the integer.
 *
 * @param bitmap pointer to the 32-bit integer
 * @param mask bits to set
 * @return previous value of the bitmap
 */
static uint32_t bitmap_or_release(uint32_t *bitmap, uint32_t mask)
{
	/* The pointer cast is fine because Linux is compiled with strict
	 * aliasing disabled
	 */
	return (uint32_t)atomic_fetch_or_release(mask, (atomic_t *)bitmap);
}

/**
 * @brief Atomically set an unsigned 32-bit integer to 0 with acquire memory
 * ordering and return the previous value of the integer.
 *
 * @param bitmap pointer to the 32-bit integer
 * @return previous value of the bitmap
 */
static uint32_t bitmap_clear_acquire(uint32_t *bitmap)
{
	/* The pointer cast is fine because Linux is compiled with strict
	 * aliasing disabled
	 */
	return (uint32_t)atomic_xchg_acquire((atomic_t *)bitmap, 0);
}
...
static void notify_ns_message(void)
{
	...
	spin_lock(&ring_lock);
	ret = lr_prod_commit(&_ns_to_s_ring);
	...
	if (ret != 0) {
		 ...
		 *  - set E_MESSAGE
		 */
		ns_notifications = bitmap_or_release(
			_ns_to_s_notification_register, E_MESSAGE);
 ...
 * to \ref notify_ns_message is done.
 *
 ...
 */
static void write_ns_message(struct pnc_message *ree_msg_ptr)
{
	 ...
	 */
	struct pnc_shm *shm_base = (struct pnc_shm *)pnc_shm_base();

	spin_lock(&ring_lock);
	shm_base->ring_ns_to_s.msgs[lr_prod_peek(&_ns_to_s_ring)] = *ree_msg_ptr;
	lr_prod_advance(&_ns_to_s_ring);
	spin_unlock(&ring_lock);
}
 ...
 */
static void handle_s_request(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
	...
	if (s->global_state == S_CONFIGURED) {
		...
		case S_IDLE:
			/* Copy S request */
			memcpy(&s->server_message, ree_msg_ptr, sizeof(struct pnc_message));
 ...
 */
static void handle_s_response(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
	...
	if (s->global_state == S_CONFIGURED) {
		...
		case S_CANCEL_WAITING:
			/* Copy S response */
			memcpy(&s->client_message, ree_msg_ptr, sizeof(struct pnc_message));
 ...
 */
static void handle_s_config_ack(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
 ...
 */
static void handle_s_cancel(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
	struct pnc_message ree_msg = { 0 };
	...
	if (s->global_state == S_CONFIGURED) {
		...
		case S_NOTIFIED:
			...
			memset(&s->server_message, 0, sizeof(struct pnc_message));
			/* Send A_CANCEL_ACK */
 ...
 */
static void handle_s_cancel_ack(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
 ...
 */
static void handle_s_term(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
	struct pnc_message ree_msg = { 0 };
	...
	if (s->global_state == S_CONFIGURED) {
		...
		bitmap_clear_acquire(&_ns_to_s_signals[s->index]);
 ...
 */
static void handle_s_term_ack(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s = &_sessions[ree_msg_ptr->index];
...
/**
 * @brief Handle new S struct pnc_message
 */
static void handle_s_message(struct pnc_message *ree_msg_ptr)
{
	struct pnc_session *s;
 ...
 */
static void handle_s_signal(struct pnc_session *s)
{
	down(&s->sem);
...
static void handle_s_notification(struct work_struct *work)
{
	 ...
	 */
	s_notifications = bitmap_clear_acquire(_s_to_ns_notification_register);
	...
	if (s_notifications & E_MESSAGE) {
		...
		shm_base = (struct pnc_shm *)pnc_shm_base();
		while (lr_cons_simple_checkout(&_s_to_ns_ring) ||
				lr_cons_final_checkout(&_s_to_ns_ring)) {
			do {
				/* Handle new S message */
				ree_msg = PNR_SAFE_READ(shm_base->ring_s_to_ns.msgs[lr_cons_peek(&_s_to_ns_ring)]);
				handle_s_message(&ree_msg);
				lr_cons_advance(&_s_to_ns_ring);
			} while (lr_cons_can_consume(&_s_to_ns_ring));
		}
...
irqreturn_t pnc_session_interrupt_handler(int irq, void *dev_id)
{
	...
	if (_session_ready == SESSION_NOT_READY) {
		...
		if (pnc_shm_ready()) {
			...
			header = (struct pnc_header *)shm_base;
...
int pnc_sessions_init(void)
{
	...
	memset(shm_base, 0, sizeof(struct pnc_shm));
	...
	/* Init shared ring buffers regions */
	pr_info("Initialising ring buffers.\n");
	lr_shrd_prod_init(&shm_base->ring_ns_to_s.prod_shared);
	lr_shrd_cons_init(&shm_base->ring_ns_to_s.cons_shared);
	lr_shrd_prod_init(&shm_base->ring_s_to_ns.prod_shared);
	lr_shrd_cons_init(&shm_base->ring_s_to_ns.cons_shared);

	/* Init private part of ring buffers */
	lr_prod_init(&_ns_to_s_ring,
				&shm_base->ring_ns_to_s.prod_shared,
				&shm_base->ring_ns_to_s.cons_shared,
				PNC_MESSAGE_RING_SLOT_CAPACITY);
	lr_cons_init(&_s_to_ns_ring,
				&shm_base->ring_s_to_ns.prod_shared,
				&shm_base->ring_s_to_ns.cons_shared,
				PNC_MESSAGE_RING_SLOT_CAPACITY);
...
void pnc_sessions_exit(void)
{
	 ...
	 */
	ns_notifications = bitmap_or_release(_ns_to_s_notification_register,
			E_RESET);
...
int pnc_session_get_mem_offset(struct pnc_session *session,
 ...
 *              for a valid session.
 */
static int check_session_configured(struct pnc_session *s)
{
	int ret;
	unsigned long flags;
	uint32_t local_sessions;
 ...
 *   - 0            otherwise
 */
static int check_and_handle_terminated_session(struct pnc_session *session)
{
...
static int wait_session_event(struct pnc_session *s, uint32_t mask, uint32_t *events,
	uint32_t timeout)
{
	int ret;
	int ret2;
	...
	ret2 = check_and_handle_terminated_session(s);
	if (ret2 == -ERESTARTSYS)
		pr_err("%s: interrupted while waiting semaphore.\n", __func__);
 ...
 */
static void send_term(struct pnc_session *s)
{
	struct pnc_message ree_msg = {0};
 ...
 */
static int send_request(struct pnc_session *s, uint32_t request)
{
	struct pnc_message ree_msg = {0};
 ...
 */
static int get_response(struct pnc_session *s, uint32_t *response)
{
	/* Acquire the lock on the session. */
	if (down_interruptible(&s->sem)) {
		pr_err("%s: interrupted while waiting semaphore.\n", __func__);
	...
	if (response) {
		*response = s->client_message.p1;
		memset(&s->client_message, 0, sizeof(struct pnc_message));
	}
	s->client_state = S_IDLE;
...
static int _session_open(struct pnc_session **session, unsigned int flags)
{
	unsigned int index;
	unsigned long lock_flags;
	uint32_t local_sessions;
	...
	for (index = 0; index < REE_MAX_SESSIONS; index++) {
		if (_sessions[index].free) {
			...
			*session = &_sessions[index];
			_sessions[index].event_pending = 0;
			memset(&_sessions[index].client_message, 0, sizeof(struct pnc_message));
			memset(&_sessions[index].server_message, 0, sizeof(struct pnc_message));
			mutex_unlock(&_sessions_mutex);
			return 0;
		}
	}
...
int pnc_session_open(struct pnc_session **session)
{
	return _session_open(session, 0);
}
EXPORT_SYMBOL(pnc_session_open);

int pnc_session_open_with_flags(struct pnc_session **session, unsigned int flags)
{
	return _session_open(session, flags);
}
EXPORT_SYMBOL(pnc_session_open_with_flags);

void pnc_session_close(struct pnc_session *session)
{
	int ret;
...
int pnc_session_get_version(struct pnc_session *session, uint32_t *version)
{
...
int pnc_session_alloc(struct pnc_session *session, unsigned long size)
{
	if (session->mem != NULL) {
...
static int configure_session(struct pnc_session *s, const char *name)
{
	int ret;
	struct pnc_message ree_msg = {0};
	unsigned long flags;
	...
	if (_ree_version < 0x307) {
		/* For such REE versions, p0 is expected to be a SID indicating session's
		 * configuration is made "by name"
		 */
		p0_param = TZ_CONFIG_ARG_GETSYSPROC_SID;
		p0_param |= 1LLU << 62;
		p0_param |= 1LLU << 63;
	} else {
		/* TODO (?): p0 is now unused and can be used for any new needed feature */
		p0_param = 0;
	}
...
int pnc_session_config_by_name(struct pnc_session *session, const char *name)
{
	return configure_session(session, name);
}
EXPORT_SYMBOL(pnc_session_config_by_name);

int pnc_session_get_mem(struct pnc_session *session, char **ptr, unsigned long *size)
{
	int ret;
...
int pnc_session_send_response(struct pnc_session *s, uint32_t response)
{
	int ret;
	struct pnc_message ree_msg = {0};
...
int pnc_session_get_response(struct pnc_session *s, uint32_t *response)
{
	int ret;
...
int pnc_session_wait_response(struct pnc_session *s, uint32_t *response,
		uint32_t timeout)
{
	int ret;
	...
	if (ret == -ERESTARTSYS && timeout != NO_TIMEOUT) {
		/* Let's make sure the kernel won't recall with the same parameters, as
...
int pnc_session_send_request(struct pnc_session *s, uint32_t request)
{
	int ret;
...
int pnc_session_get_request(struct pnc_session *s, uint32_t *request)
{
	int ret;
...
int pnc_session_wait_request(struct pnc_session *s, uint32_t *request,
		uint32_t timeout)
{
	int ret;
...
int pnc_session_cancel_request(struct pnc_session *s, uint32_t *response,
	uint32_t timeout)
{
	int ret;
	struct pnc_message ree_msg = {0};
...
int pnc_session_send_request_and_wait_response(struct pnc_session *session,
...
int pnc_session_send_signal(struct pnc_session *s, uint32_t bits)
{
	int ret;
	uint32_t signals, ns_notifications;

	if (bits == 0) {
		pr_err("(%s) Will not send empty signals bitfield.\n", __func__);
		return -EPROTO;
	}
	 ...
	 */
	signals = bitmap_or_release(&_ns_to_s_signals[s->index], bits);
...
int pnc_session_get_signal(struct pnc_session *s, uint32_t *signals)
{
	int ret;
	...
	*signals = bitmap_clear_acquire(&_s_to_ns_signals[s->index]);

	return 0;
}
EXPORT_SYMBOL(pnc_session_get_signal);

int pnc_session_wait_signal(struct pnc_session *s, uint32_t *signals,
		uint32_t timeout)
{
	int ret;
...
int pnc_session_wait_event(struct pnc_session *s, uint32_t *events, uint32_t mask,
		uint32_t timeout)
{
	int ret;
	...
	ret = wait_session_event(s, mask, events, timeout);
	if (ret == -ERESTARTSYS && timeout != NO_TIMEOUT) {
		/* Let's make sure the kernel won't recall with the same parameters, as
		 * timeout would be wrong
		 */
...
__poll_t pnc_session_poll_wait(struct pnc_session *session, struct file *file,
				       poll_table *wait)
{
	int ret;
	...
	/* Need to check if session broken or terminated while waiting response */
	ret = check_and_handle_terminated_session(session);
	if (ret == -EPIPE)
		return EPOLLHUP;
...
int pnc_session_get_pending_events(struct pnc_session *session, uint32_t *events)
{
	int ret;
