...
//! JSON Transport using `serde_json` for serialization
...
pub struct JsonSender<S: BytesSender> {
    inner: S,
}

impl<T, S> Sender<T> for JsonSender<S>
where
    T: serde::Serialize,
    S: BytesSender,
{
...
pub struct JsonReceiver<R: BytesReceiver> {
    inner: R,
}

impl<T, R> Receiver<T> for JsonReceiver<R>
where
    T: for<'de> serde::Deserialize<'de>,
    R: BytesReceiver,
{
    fn builder() -> fn(me: s_pid_t, other: s_pid_t) -> Self {
        |me, other| JsonReceiver {
    ...
    fn recv(&self) -> Result<T> {
        ...
        serde_json::from_slice(&buf)
            .map_err(|e| Error::msg(&format!("Json Deserialization failed: {}", e)))
    }
}
